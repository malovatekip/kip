...
def start_game(
    ...
    old = db.execute(text(
        "SELECT id FROM bizsim_sessions WHERE user_id=:uid"
...
async def run_day(
    ...
    # Loan interest (any phase, since loans now available from Phase 2)
    interest = 0
    if s["loan_balance"] > 0:
    ...
    if day % 7 == 0:
        ...
        coaching     = await generate_ai_coaching(
            s["biz_type"], day // 7, week_history, bs, s["phase"], lang
        )
...
def repay_loan(
    ...
    """Repay all or part of an outstanding loan from cash on hand."""
    ensure_tables(db)
    row = _get_session(current_user.id, db)
    if not row:
        raise HTTPException(404, "No active session.")
    ...
    if s["loan_balance"] <= 0:
        raise HTTPException(400, "You have no outstanding loan.")
    if req.amount <= 0:
        raise HTTPException(400, "Repayment amount must be greater than 0.")
    if req.amount > s["cash"]:
        raise HTTPException(400, f"You only have K{s['cash']:,.0f} in cash.")

    repay_amount = min(req.amount, s["loan_balance"])
    new_balance  = round(s["loan_balance"] - repay_amount, 2)
    new_rate     = s["loan_interest_rate"] if new_balance > 0 else 0

    record_transaction(db, s["id"], "Loan repayment", [
        {"account": "loan_payable", "debit":  repay_amount, "credit": 0},
        {"account": "cash",         "debit":  0,            "credit": repay_amount},
    ], s["day"])
    ...
    db.execute(text("""
        ...
        WHERE id=:sid
    """), {
        "amt":  repay_amount, "bal": new_balance,
        "rate": new_rate, "now": datetime.utcnow().isoformat(), "sid": s["id"],
    })
    db.commit()
    ...
    return {
        ...
        "message": (
            "Loan fully repaid! You can apply for a new loan if needed."
            if new_balance == 0
            else f"Repaid K{repay_amount:,.0f}. Remaining balance: K{new_balance:,.0f}."
        ),
    }
