...
import {
  User, Globe, CreditCard, Sun, Moon, AlertTriangle, X,
} from 'lucide-react'
...
function DeleteAccountModal({ onClose }) {
  ...
  const handleDelete = async e => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || t('settings.error_delete_account'))
      setDeleting(false)
    }
  }
    ...
    <div style={{
      ...
      <div className="card animate-scale-in" onClick={e => e.stopPropagation()} style={{
        ...
        <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: 'var(--space-5)' }}>
          {t('settings.delete_detail_p1')} <strong>{t('settings.delete_detail_strong')}</strong> {t('settings.delete_detail_p2')}
        </p>

        <form onSubmit={handleDelete}>
          <div className="input-group" style={{ marginBottom: 'var(--space-5)' }}>
            <label className="input-label">{t('settings.confirm_password_label')}</label>
            <input
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', justifyContent: 'flex-end' }}>
            ...
            <button type="submit" className="btn btn-danger btn-sm" disabled={deleting || !password}>
              {deleting ? t('settings.deleting_ellipsis') : t('settings.delete_my_account')}
            </button>
          </div>
        </form>
      </div>
  ...
  return (
    <Layout>
      <div className="content-area page-enter" style={{ maxWidth: 700 }}>
        ...
        {/* Profile */}
        <Section icon={User} title={t('settings.profile_title')} colour="blue">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-4)' }}>
            ...
            <div className="input-group">
              <label className="input-label">{t('settings.email_address_label')}</label>
          ...
          <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 'var(--space-3)', marginBottom: 0 }}>
            {t('settings.profile_editing_soon')}
          </p>
        </Section>
        ...
        <Section icon={Globe} title={t('settings.language_section')} colour="gold">
          ...
          <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', lineHeight: 1.6, margin: 0 }}>
            {t('settings.language_choose_desc')}
          </p>
        </Section>

        {/* Appearance */}
        <Section icon={Sun} title={t('settings.appearance_section')} colour="copper">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-6)' }}>
            <div>
              <div style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--text-primary)' }}>{t('settings.theme_label')}</div>
              <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 2 }}>{t('settings.theme_desc')}</div>
            </div>
            <button onClick={toggle} style={{
              ...
              {theme === 'dark' ? <><Sun size={13} /> {t('settings.switch_to_light')}</> : <><Moon size={13} /> {t('settings.switch_to_dark')}</>}
            </button>
          </div>
        </Section>

        {/* Subscription — KIP is free for everyone right now, no upgrade CTA */}
        <Section icon={CreditCard} title={t('settings.subscription_title')} colour="gold">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--space-4)' }}>
            <span className="badge badge-blue" style={{ fontSize: 12, padding: '4px 12px', textTransform: 'capitalize' }}>
              {user?.plan_tier || 'free'} {t('settings.plan_suffix')}
            </span>
          </div>
        </Section>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--space-4)' }}>
            <div>
              ...
              <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', margin: 0 }}>
                {t('settings.delete_account_warning_short')}
              </p>
            </div>
