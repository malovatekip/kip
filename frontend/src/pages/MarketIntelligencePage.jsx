...
const FIELD_LABELS = {
  avg_avg_shop_rent_pm:        { label: 'Avg Shop Rent',      icon: Building2,   format: v => `K${v.toLocaleString()}/mo`  },
  avg_avg_labor_wage_pm:       { label: 'Avg Labour Wage',    icon: Users,       format: v => `K${v.toLocaleString()}/mo`  },
  avg_avg_tomato_price_per_kg: { label: 'Tomatoes (per kg)',  icon: ShoppingBag, format: v => `K${v.toFixed(2)}`           },
  avg_avg_bread_price:         { label: 'Bread (loaf)',       icon: ShoppingBag, format: v => `K${v.toFixed(2)}`           },
  avg_avg_phone_data_1gb:      { label: '1GB Data',           icon: Zap,         format: v => `K${v.toFixed(2)}`           },
  most_common_foot_traffic:    { label: 'Foot Traffic',       icon: Users,       format: v => v.replace(/_/g,' ')          },
  most_common_dominant_income_level:{ label: 'Income Level', icon: DollarSign,  format: v => v.replace(/_/g,' ')          },
  most_common_competition_quality:  { label: 'Competition',  icon: TrendingUp,  format: v => v.replace(/_/g,' ')          },
  most_common_area_type:       { label: 'Area Type',          icon: MapPin,      format: v => v.replace(/_/g,' ')          },
}
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: 'var(--muted)' }}>
            {subs} survey submission{subs !== 1 ? 's' : ''}
          </div>
        </div>
        <div style={{
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {agg.most_common_foot_traffic && (
          <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: 'var(--teal-dim)', color: 'var(--teal)', fontFamily: 'Syne', fontWeight: 600 }}>
            {agg.most_common_foot_traffic.replace(/_/g,' ')} traffic
          </span>
        )}
        ...
        {agg.most_common_dominant_income_level && (
          <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: 'var(--gold-dim)', color: 'var(--gold)', fontFamily: 'Syne', fontWeight: 600 }}>
            {agg.most_common_dominant_income_level.replace(/_/g,' ')} income
          </span>
        )}
      </div>
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ paddingBottom: 14, borderBottom: '1px solid var(--border)' }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--muted)', margin: 0 }}>
          Aggregated from {town.submissions} survey submission{town.submissions !== 1 ? 's' : ''} ·
          Last updated {town.last_updated ? new Date(town.last_updated).toLocaleDateString('en-ZM', { day: 'numeric', month: 'long', year: 'numeric' }) : '—'}
        </p>
      </div>
      ...
      {stats.length > 0 && (
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
            {stats.map(({ key, label, icon: Icon, format, value }) => (
              <div key={key} style={{ background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 12, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 5 }}>
                  ...
                  <span style={{ fontSize: 10, fontFamily: 'Syne', fontWeight: 600, color: 'var(--muted)' }}>{label}</span>
                </div>
      ...
      {gaps.length > 0 && (
        <div>
          <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 12, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
            Market Gaps Identified by Locals
          </div>
      ...
      {agg.competitor_weaknesses?.length > 0 && (
        <div>
          <div style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 12, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 10 }}>
            Competitor Weaknesses Reported
          </div>
      ...
      <div style={{ display: 'flex', gap: 8, padding: '10px 14px', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 10 }}>
        ...
        <span style={{ fontSize: 12, color: 'var(--faint)', lineHeight: 1.6 }}>
          All data is anonymised and aggregated from KIP user surveys. Individual responses cannot be traced. Figures are indicative — always verify locally before making business decisions.
        </span>
      </div>
    </div>
  )
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '20px 16px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 22 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.65, maxWidth: 620 }}>
            Real, crowd-sourced market data from KIP users across Zambia. Prices, rents, competition levels, and market gaps — by town.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="kip-card" style={{ padding: '52px 24px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: 13, color: 'var(--muted)', maxWidth: 400, margin: '0 auto 20px' }}>
              Market intelligence grows as KIP users submit area surveys. Be the first to add data for your town.
            </p>
            <a href="/survey" style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '10px 20px', borderRadius: 10, background: 'var(--teal-dim)', border: '1px solid rgba(0,240,200,0.3)', color: 'var(--teal)', textDecoration: 'none', fontFamily: 'Syne', fontWeight: 700, fontSize: 13 }}>
              <MapPin size={14} /> Submit Area Survey
            </a>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.6fr)', gap: 20, alignItems: 'start' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 520, overflowY: 'auto' }}>
                {filtered.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 24, fontSize: 13, color: 'var(--faint)' }}>No towns match "{search}"</div>
                ) : filtered.map(town => (
              ...
              <div style={{ marginTop: 12, padding: '10px 14px', background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 10, fontSize: 12, color: 'var(--muted)', lineHeight: 1.6 }}>
                <strong style={{ color: 'var(--text)' }}>Help build this map.</strong> Every time you submit a market survey for your area, it adds to the intelligence here — benefiting every entrepreneur in that town.
              </div>
            </div>
            ...
            <div className="kip-card" style={{ padding: '20px 22px' }}>
              {selected
                ...
                : <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--faint)', fontSize: 13 }}>Select a town to view market data</div>
              }
            </div>
          </div>
        )}
